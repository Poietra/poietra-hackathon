...
export async function saveRoomImage(room: string, bytes: Uint8Array<ArrayBuffer>): Promise<string> {
  if (!imageMime(bytes)) throw new Error('PNG・JPEG・WebP の画像を選択してください。');
  const directory = resolve(process.env.POIETRA_DATA_DIR || '.data', 'images', room);
  ...
  return `/api/rooms/${room}/images/${id}`;
}
...
export async function handleImages(request: IncomingMessage, response: ServerResponse, pathname: string): Promise<boolean> {
  ...
  try {
    ...
    json(200, { src: await saveRoomImage(route[1], new Uint8Array(Buffer.concat(chunks))) });
