...
検証では、別ブラウザ間の共同編集・オフライン編集の合流・取り消し、実 workerd の強制休止と再起動後の復元、キャンバス・タイムライン・ロック・レイヤー順・ファイル読み込みを確認します。AI は API を代替したテストで提案の競合、キャンセル、再試行を検証しています。実 API 呼び出しはキー設定後に確認します。
