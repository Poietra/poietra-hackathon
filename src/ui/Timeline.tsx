...
function TrackRow({ track }: { track: AnimationTrack }) {
  ...
  return <div data-track-object-id={track.objectId} className={`track-row ${selectedIds.includes(track.objectId) ? 'selected' : ''}`}>
...
export function Timeline({ zoom, setZoom }: { zoom: number; setZoom: (value: number) => void }) {
  ...
  return <section className={`timeline ${transition ? 'expanded' : ''}`} aria-label="タイムライン">
    ...
    {transition ? <div className="transition-tracks"><div className="transition-caption"><span>Transition</span><span>{ms(transition.duration)} ms</span><button aria-label="Composition に戻る" onClick={() => select({ kind: 'composition', id: transition.toId })}><ChevronDown size={14}/></button></div><TransitionRuler duration={transition.duration} local={local} start={segment!.start}/><div className="track-rows"><div className="track-grid">{Array.from({ length: 5 },(_,i)=><i key={i} style={{ left: `${i*25}%` }}/>)}</div>{animations.map(({ track }) => <TrackRow key={track.objectId} track={track}/>)}{animations.length === 0 && <p className="tracks-empty">Composition にオブジェクトを追加すると、動きを調整できます</p>}<div className="track-playhead" style={{ left: `calc(var(--track-label-width, 190px) + (100% - var(--track-label-width, 190px)) * ${fraction(local, transition.duration)})` }}/></div></div> : <div className="scene-ruler">{segments.map(part => <span key={part.id} style={{ left: `${fraction(part.start, total) * 100}%` }}><i/>{ms(part.start)}</span>)}<span className="ruler-end"><i/>{ms(total)} ms</span><input type="range" aria-label="再生位置" min={0} max={Math.max(1,total)} step={1} value={Math.min(playhead,total)} onChange={e => seek(Number(e.target.value), 'scene')}/></div>}
