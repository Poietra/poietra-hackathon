...
import { ArrowRight, ChevronDown, CornerDownRight, LockKeyhole, Pause, Play, Plus, SkipBack, Square } from 'lucide-react';
import { useEditor } from '../editor/context';
import { ANIMATIONS, PROPERTY_CHANNELS, PROPERTY_CHANNEL_LABELS, clamp, getPropertyTiming, hasPropertyTiming, ms, sceneDuration, sceneSegments, type AnimationTrack, type PropertyChannel } from '../../shared/model';
...
function TrackRow({ track, channel, report, active }: { track: AnimationTrack; channel?: PropertyChannel; report: (status: OperationStatus | null) => void; active: string | null }) {
  ...
  const timing = channel ? getPropertyTiming(track, channel) : track;
  const label = channel ? PROPERTY_CHANNEL_LABELS[channel] : ANIMATIONS[track.type];
  ...
  }, [scene.id, selection.id, track.objectId, channel, transition?.duration, object?.locked, playing, store]);
  ...
  function down(event: PointerEvent<HTMLButtonElement>) {
    ...
    if (channel && !hasPropertyTiming(current.track, channel)) return;
    const timing = channel ? getPropertyTiming(current.track, channel) : current.track;
    ...
    drag.current = {
      ...
      lastStart: timing.start, lastDuration: timing.duration, materialized: channel ? hasPropertyTiming(current.track, channel) : current.existing, undoBefore: store.undoManager.undoStack.at(-1), element: event.currentTarget,
    };
    store.beginGesture();
    try { event.currentTarget.setPointerCapture(event.pointerId); }
    catch { finish(true); return; }
    event.preventDefault();
    report({ kind: 'active', label: `${object.name}${channel ? ` · ${label}` : ''} · ${edge === 'start' ? '開始を調整' : edge === 'end' ? '終了を調整' : '時間を移動'}`, detail: `${ms(timing.start)}–${ms(timing.start + timing.duration)} ms` });
  }
  function move(event: PointerEvent<HTMLButtonElement>) {
    ...
    if (!current || current.object.locked || initial.materialized && !(channel ? hasPropertyTiming(current.track, channel) : current.existing) || currentTransition.duration !== initial.total) { finish(true); return; }
    const currentTrack = channel ? getPropertyTiming(current.track, channel) : current.track;
    ...
    try {
      if (channel) store.setPropertyTiming(initial.sceneId, initial.transitionId, initial.objectId, channel, { start, duration, easing: currentTrack.easing }, false);
      else store.setTrack(initial.sceneId, initial.transitionId, initial.objectId, patch, false);
      ...
      report({ kind: 'active', label: `${object.name}${channel ? ` · ${label}` : ''} · ${initial.edge === 'start' ? '開始を調整' : initial.edge === 'end' ? '終了を調整' : '時間を移動'}`, detail: `${ms(start)}–${ms(start + duration)} ms · Esc で戻す` });
    } catch { finish(true); }
  }
  ...
  return <div data-track-object-id={channel ? undefined : track.objectId} data-property-object-id={channel ? track.objectId : undefined} data-property-channel={channel} data-track-active={active === track.objectId || undefined} className={cn('track-row', channel && 'property-track-row', selectedIds.includes(track.objectId) && 'selected', disabled && 'track-unavailable')}>
    <button className="track-label" aria-label={channel ? `${object.name} ${label}` : undefined} aria-pressed={selectedIds.includes(track.objectId)} onClick={() => { report(null); setSelectedIds([track.objectId]); }}>{channel ? <CornerDownRight size={12}/> : <ObjectIcon kind={object.kind}/>}<span>{channel ? label : object.name}</span>{object.locked && <LockKeyhole size={12} aria-label="ロック中"/>}{!channel && <small>{label}</small>}</button>
    <div className="track-lane" ref={lane}>
      <button className={cn('animation-bar', selectedIds.includes(track.objectId) && 'selected')} style={{ left: `min(${fraction(timing.start, transition.duration) * 100}%, calc(100% - 6px))`, width: `max(6px, ${fraction(Math.min(timing.duration, Math.max(0, transition.duration - timing.start)), transition.duration) * 100}%)`, cursor: disabled ? 'not-allowed' : undefined }}
        aria-label={`${object.name} ${label}: ${ms(timing.start)}–${ms(timing.start + timing.duration)} ms`} aria-disabled={object.locked || playing || transition.duration <= 0}
        ...
        <span data-edge="start" className="bar-handle start"/><span className="bar-name">{label}<span className="bar-timing"> · {ms(timing.start)}–{ms(timing.start + timing.duration)} ms</span></span><span data-edge="end" className="bar-handle end"/>
      </button>
    </div>
  </div>;
}
...
export function Timeline({ zoom, setZoom, children }: { zoom: number; setZoom: (value: number) => void; children?: ReactNode }) {
  ...
  return <section className={`timeline ${transition ? 'expanded' : ''}`} aria-label="タイムライン">
    ...
    {transition ? <div className="transition-tracks"><div className="transition-caption"><span>Transition</span><span>{ms(transition.duration)} ms</span><OperationFeedback status={feedback || idleStatus} className="timeline-feedback"/><button aria-label="Composition に戻る" onClick={() => select({ kind: 'composition', id: transition.toId })}><ChevronDown size={14}/></button></div><TransitionRuler duration={transition.duration} local={local} start={segment!.start}/><div className="track-rows"><div className="track-grid">{Array.from({ length: 5 },(_,i)=><i key={i} style={{ left: `${i*25}%` }}/>)}</div>{animations.map(({ track }) => <Fragment key={track.objectId}><TrackRow track={track} report={report} active={activeTrack}/>{PROPERTY_CHANNELS.filter(channel => hasPropertyTiming(track, channel)).map(channel => <TrackRow key={channel} track={track} channel={channel} report={report} active={activeTrack}/>)}</Fragment>)}{animations.length === 0 && <p className="tracks-empty">Composition にオブジェクトを追加すると、動きを調整できます</p>}<div className="track-playhead" style={{ left: `calc(var(--track-label-width, 190px) + (100% - var(--track-label-width, 190px)) * ${fraction(local, transition.duration)})` }}/></div></div> : <div className="scene-ruler">{segments.map(part => <span key={part.id} style={{ left: `${fraction(part.start, total) * 100}%` }}><i/>{ms(part.start)}</span>)}<span className="ruler-end"><i/>{ms(total)} ms</span><input type="range" aria-label="再生位置" min={0} max={Math.max(1,total)} step={1} value={Math.min(playhead,total)} onChange={e => seek(Number(e.target.value), 'scene')}/></div>}
    {children}
