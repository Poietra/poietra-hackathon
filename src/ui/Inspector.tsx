...
import { AlignHorizontalJustifyStart, AlignHorizontalJustifyCenter, AlignHorizontalJustifyEnd, AlignVerticalJustifyStart, AlignVerticalJustifyCenter, AlignVerticalJustifyEnd, Link2, Unlink, Eye, EyeOff, Plus, RotateCcw, Spline, X, Copy, LockKeyhole, UnlockKeyhole } from 'lucide-react';
import { useEditor } from '../editor/context';
import { ANIMATIONS, EASINGS, KINDS, COLORS, defaultTrack, type AnimationKind, type Easing, type ObjectState } from '../../shared/model';
...
function ColorInput({ value, onChange, label }: { value: string; onChange: (color: string) => void; label: string }) {
  ...
  return <div className="color-field"><label className="color-swatch" style={{ background: value }}><input aria-label={`${label}の色を選択`} type="color" value={value} onChange={e => onChange(e.target.value)}/></label><input aria-label={`${label}のカラーコード`} value={draft ?? value.replace('#','').toUpperCase()} onFocus={() => setDraft(value.replace('#','').toUpperCase())} onChange={e => setDraft(e.target.value)} onBlur={() => { if (draft && /^#?[\da-f]{6}$/i.test(draft)) onChange('#' + draft.replace('#','')); setDraft(null); }} onKeyDown={e => { if (e.key === 'Enter') e.currentTarget.blur(); }}/><span>100<span className="muted"> %</span></span></div>;
}
...
export function Inspector() {
  ...
  function update(patch: Partial<ObjectState>) { for (const id of selectedIds) store.updateState(scene.id, compositionId, id, patch); }
  ...
  if (transition) {
    ...
    function setTrack(patch: Parameters<typeof store.setTrack>[3]) { if (object) store.setTrack(scene.id, transition!.id, object.id, patch); }
    ...
    return <div className="inspector-content"><div className="inspector-title"><span>{object?.name || 'Transition'}</span><span className="inspector-kind">{object ? KINDS[object.kind] : 'Between compositions'}</span></div>
      <Section title="Transition"><Field label="Duration"><NumberInput value={transition.duration} onChange={value => store.setTransitionDuration(scene.id, transition.id, value)} label="Transition duration" suffix="ms" min={100} max={120000}/></Field></Section>
      {object && timing ? <>
        ...
        <Section title="Presence"><p className="muted small">{entering ? `${scene.compositions[transition.toId]?.name} only` : leaving ? `${scene.compositions[transition.fromId]?.name} only` : 'Both compositions'}</p><Field label="From"><select aria-label="Present in source composition" value={from?.visible ? 'visible' : 'none'} onChange={e => store.updateState(scene.id, transition.fromId, object.id, { visible: e.target.value === 'visible' })}><option value="visible">Visible</option><option value="none">None</option></select></Field><Field label="To"><select aria-label="Present in destination composition" value={to?.visible ? 'visible' : 'none'} onChange={e => store.updateState(scene.id, transition.toId, object.id, { visible: e.target.value === 'visible' })}><option value="visible">Visible</option><option value="none">None</option></select></Field></Section>
      </> : <div className="inspector-empty"><Spline size={28}/><p>レイヤーを選択して<br/>個々の動きを調整</p></div>}
    </div>;
  }

  return <div className="inspector-content"><div className="inspector-title"><span>{selectedIds.length > 1 ? `${selectedIds.length} objects` : object ? KINDS[object.kind] : 'Composition'}</span>{object && <input className="object-name" aria-label="Object name" value={object.name} onChange={e => store.setObject(scene.id, object.id, { name: e.target.value })}/>}</div>
    {selectedIds.length > 0 ? <>
      {state && <Section title="Position"><div className="property-grid"><Field label="X"><NumberInput value={state.x} label="Position X" onChange={x => update({ x })}/></Field><Field label="Y"><NumberInput value={state.y} label="Position Y" onChange={y => update({ y })}/></Field></div><div className="property-grid"><Field label="↳"><NumberInput value={state.rotation} label="Rotation" onChange={rotation => update({ rotation })} suffix="°" min={-360} max={360}/></Field><div className="button-group"><IconButton label="90度回転" onClick={() => update({ rotation: (state.rotation + 90) % 360 })}><RotateCcw size={14}/></IconButton><IconButton label={object!.locked ? 'ロック解除' : 'ロック'} active={object!.locked} onClick={() => store.setObject(scene.id, object!.id, { locked: !object!.locked })}>{object!.locked ? <LockKeyhole size={13}/> : <UnlockKeyhole size={13}/>}</IconButton></div></div></Section>}
      <Section title={selectedIds.length > 1 ? 'Alignment' : 'Align'}><div className="alignment-buttons">{([[AlignHorizontalJustifyStart,'x','start','左揃え'],[AlignHorizontalJustifyCenter,'x','center','左右中央'],[AlignHorizontalJustifyEnd,'x','end','右揃え'],[AlignVerticalJustifyStart,'y','start','上揃え'],[AlignVerticalJustifyCenter,'y','center','上下中央'],[AlignVerticalJustifyEnd,'y','end','下揃え']] as const).map(([Icon,axis,position,label]) => <IconButton key={label} label={label} onClick={() => align(axis,position)}><Icon size={14}/></IconButton>)}</div>{selectedIds.length > 1 && <button className="subtle-button full-width" onClick={() => { store.link(scene.id, selectedIds); editor.notify('オブジェクトを連結しました'); }}><Link2 size={14}/>Link objects</button>}{selectedIds.some(id => scene.objects[id]?.groupId) && <button className="text-button" onClick={() => store.unlink(scene.id, selectedIds)}><Unlink size={13}/>Unlink objects</button>}</Section>
      {state && <><Section title="Size"><div className="property-grid"><Field label="W"><NumberInput value={state.width} label="Width" onChange={width => update({ width })} min={1}/></Field><Field label="H"><NumberInput value={state.height} label="Height" onChange={height => update({ height })}/></Field></div></Section>
      {(object?.kind === 'text' || object?.kind === 'equation') && <Section title={object.kind === 'equation' ? 'LaTeX' : 'Text'}><textarea className="content-input" aria-label={object.kind === 'equation' ? 'LaTeX expression' : 'Text content'} value={state.text} onChange={e => update({ text: e.target.value })} rows={3} spellCheck={false}/><Field label="Size"><NumberInput value={state.fontSize} label="Font size" onChange={fontSize => update({ fontSize, ...(object.kind === 'equation' ? { width: state.width * fontSize / state.fontSize, height: state.height * fontSize / state.fontSize } : {}) })} min={8} max={400} suffix="px"/></Field></Section>}
      ...
      <Section title="Fill"><ColorInput value={state.fill} onChange={fill => update({ fill })} label="Fill"/><div className="palette">{COLORS.map(color => <button key={color} aria-label={`色 ${color}`} onClick={() => update({ fill: color })} style={{ background: color }} className={state.fill === color ? 'selected' : ''}/>)}</div></Section>
      ...
      <Section title="Actions"><button className="subtle-button full-width" onClick={() => editor.setSelectedIds(store.duplicate(scene.id, compositionId, selectedIds))}><Copy size={13}/>Duplicate<span className="shortcut">⌘D</span></button><button className="text-button" onClick={() => { store.hide(scene.id, compositionId, selectedIds); editor.setSelectedIds([]); }}><EyeOff size={13}/>Hide in this composition</button></Section>
