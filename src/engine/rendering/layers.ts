...
export class LayerCache {
  ...
  async get(item: RenderObject, scale: number, glow: GlowRenderer, signal: AbortSignal): Promise<RasterLayer | null> {
    ...
    const key = JSON.stringify([item.object.kind, item.object.image?.src, item.object.media?.src, item.videoTimeMs, local.state, item.writeProgress, item.order, scale, hasGlow]);
