...
- AI の編集案が検証で退けられた場合は、その出力と検証エラーを添えて 1 回だけ再生成する（2026-09-15）。API エラー・出力の途中終了・拒否は再生成しない。1 回の呼び出しは 60 秒で SDK の再試行は 1 回、部屋の AI ロックは 180 秒。処理時間・試行回数・トークン量をログに残す。⌘/Ctrl + Enter で送った `@codex` の依頼は、検証を通れば確認なしに適用し、Undo で戻せる。
