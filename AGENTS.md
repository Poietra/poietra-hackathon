...
- 応答速度のため、Responses API に `reasoning.effort=low` と `service_tier=fast` を既定で送る（2026-09-15）。モデルや契約が 400 で拒否した場合は同じ依頼内で既定値に戻し、以降のリクエストでは送らない。画像 2 枚は並列に生成する。`OPENAI_REASONING_EFFORT` / `OPENAI_SERVICE_TIER` で変更する。
